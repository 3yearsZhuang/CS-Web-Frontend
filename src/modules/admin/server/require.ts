...
export function requireRoot(req: Request):
  | { ok: true; user: SafeUser }
  | { ok: false; response: NextResponse } {
...
export function requireModuleAdmin(req: Request, module: AdminModule):
  | { ok: true; user: SafeUser }
  | { ok: false; response: NextResponse } {
  const admin = requireAdmin(req);
  if (!admin.ok) return admin;
...
export function requirePasswordConfirmation(
  ...
  const db = getDb();
  const row = db.prepare('SELECT password_hash FROM users WHERE id = ?').get(admin.user.id) as
