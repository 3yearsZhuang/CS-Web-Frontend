/**
 * @file 个人主页论坛 Tab — 我的主题/回复/收藏（三段 sub-tab 切换）
 */
...
/** Sub-tab 类型 */
type ForumSubTab = 'topics' | 'replies' | 'favorites';

/** Sub-tab 配置 */
const SUB_TABS: { value: ForumSubTab; label: string; num: string }[] = [
  { value: 'topics', label: 'Topics', num: '01' },
  { value: 'replies', label: 'Replies', num: '02' },
  { value: 'favorites', label: 'Favorites', num: '03' },
];

interface ProfileForumTabProps {
  /** 当前登录用户 ID */
  userId: string;
}
...
export function ProfileForumTab({ userId }: ProfileForumTabProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (activeSubTab === 'topics') {
        url = `/api/community/forum/users/${userId}/topics?page=${page}&page_size=${PAGE_SIZE}`;
      } else if (activeSubTab === 'replies') {
        url = `/api/community/forum/users/${userId}/replies?page=${page}&page_size=${PAGE_SIZE}`;
      } else {
        url = `/api/community/forum/favorites?page=${page}&page_size=${PAGE_SIZE}`;
      }
  ...
  /** 切换 sub-tab — 重置分页 */
  const handleTabChange = (tab: ForumSubTab) => {
    ...
    <div className="grid grid-cols-12 gap-0 border-t border-[var(--border)]">
      <div className="col-span-12 md:col-span-2 mb-6 md:mb-0 md:pr-6">
        ...
        <SectionNav
          ...
          value={activeSubTab}
          onChange={(v) => handleTabChange(v as ForumSubTab)}
