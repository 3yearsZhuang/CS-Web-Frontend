/**
 * @file Markdown 渲染器 — 论坛主题/回复内容（rehype-sanitize + highlight.js）
 */
...
export function MarkdownRenderer({ content, className = '' }: MarkdownRendererProps) {
  ...
  return (
    <div
      className={`forum-markdown text-[clamp(14px,1.2vw,16px)] leading-[1.8] text-[var(--foreground)] ${className}`}
    >
      <ReactMarkdown
        ...
        components={{
          ...
          // 图片
          img: ({ src, alt }) => (
            // eslint-disable-next-line @next/next/no-img-element -- 论坛图片为用户上传，next/image 需配置 remotePatterns
            <img
