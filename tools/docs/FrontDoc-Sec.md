...
> 关联：**后端鉴权/RBAC/密码/2FA/限流/审计日志权威见 [CS-Web-Backend/docs/BackDoc-Sec.md](../../CS-Web-Backend/docs/BackDoc-Sec.md)**；权限矩阵与部署模型见 [FrontDoc-Arch.md](FrontDoc-Arch.md)；运维见 [FrontDoc-Ops.md](FrontDoc-Ops.md)；演进路线 ADR 见 [FrontDoc-Evo.md](FrontDoc-Evo.md)
...
> - **后端层（见后端 `docs/BackDoc-Sec.md`）**：JWT 签发与校验、密码哈希（bcrypt）、TOTP/2FA 加密与验证、RBAC `require_permission(resource, action)` 强制、速率限制、审计日志写入、session/refresh_token 表
...
| 2 | 2FA 设置端点缺少速率限制 | 🟡中 | `api/auth/2fa/setup` `disable` | **[后端]** | 原 `twoFactorSetupLimiter`（3 次/分/IP+用户）为前端内存限流；迁移后由后端限流（见后端 `docs/BackDoc-Sec.md` §三） | ✅ |
...
| 6 | 密码哈希实现 | - | `auth/server/identity.ts` | **[遗留]**→**[后端]** | 前端 `scryptSync` 为遗留代码；运行时密码哈希由后端 bcrypt（见后端 `docs/BackDoc-Sec.md` §一） | ✅ 良好 |
...
| # | 发现 | 等级 | 位置（审计时） | 当前责任层 | 修复 | 状态 |
|---|------|------|------|:---:|------|:---:|
| 23 | 文件上传安全 | - | `community/server/forum/uploads.ts` | **[后端]** | 上传校验（大小/MIME/扩展名/魔数/文件名随机化/路径遍历防护）由后端执行；BFF 薄转发 FormData | ✅ 良好 |
| 24 | 论坛图片读取无访问控制 | 🟡中 | `api/community/forum/images/[filename]` | **[后端]** | 图片访问鉴权由后端执行；BFF 转发时注入 JWT | ✅ |
...
> ⚠️ **迁移后注意**：上表"行动"列描述的是单体时代的修复动作。迁移到 BFF 后，涉及 [后端] 责任层的控制（2FA 限流、HKDF、`requireModuleAdmin`、论坛图片鉴权、失败登录记录等）真实运行时位置在后端，前端代码为遗留。后续清理遗留代码时，对应"行动"描述需同步更新为后端实现引用。
...
| 普通管理员 | `admin` 角色（后端 RBAC 表） | `/auth/me` `roles` 数组含 `admin` | 用户管理、活动、通知群发、论坛管理 |
| 内容管理员 | `content_moderator` | `roles` 数组含 `content_moderator` | 论坛审核 |
...
> 角色数据真实存储与权限分配见后端 `docs/BackDoc-Sec.md` §一与 `docs/BackDoc-Arch.md` RBAC 章节。
...
后端 `logAdminAction` 覆盖：用户管理（update/delete/disable/enable/reset）、群发通知、活动管理、密码重置审批、公告管理、论坛版块/主题/回复审核、入社申请审批、考试/题目管理、资源审核、任务管理/认领审核、活动签到核销、删除审计日志（自我审计）。
...
> 完整端点鉴权见 [FrontDoc-Arch.md](FrontDoc-Arch.md) Part B（管理后台 §7）；后端权限点定义见后端 `docs/BackDoc-Sec.md` §一。
...
> 承接 [FrontDoc-Evo.md](FrontDoc-Evo.md) ADR-013 / ADR-014 / R7 / R8。
...
- **后端 2FA 端点**：`/auth/2fa/verify` `/auth/2fa/setup` `/auth/2fa/disable` `/auth/2fa/backup-codes`，详见后端 `docs/BackDoc-Sec.md`
- **BFF 转发行为**：
...
> 关联：[FrontDoc-Arch.md](FrontDoc-Arch.md) 部署模型、[FrontDoc-Evo.md](FrontDoc-Evo.md) ADR-015 / R7 / R8。
> **责任层说明**：本记录于单体时代生成，描述的修复动作多涉及前端遗留代码。迁移后对应控制的真实运行时位置见 Part A"责任层"列。

## 4.1 变更记录包

> ℹ️ 安全加固变更记录（第一轮/第二轮共 20 项）已迁移至根目录 `项目演变历史.md`。
...
| 生产关键变量强制配置 | ✅ | 无 | **[BFF]+[后端]** |
| TOTP 密钥派生 | ✅ | 无 | **[后端]** |
| 细粒度角色模块级 enforce | ✅ | event/blog/resource/notification/join 仅 admin/root（设计如此） | **[后端]** |
...
> ℹ️ 例外登记条目已迁移至根目录 `项目待办事项.md`。

## 4.4 标准更新积压（Backlog）

> ℹ️ 标准更新积压（Backlog）条目已迁移至根目录 `项目待办事项.md`。
