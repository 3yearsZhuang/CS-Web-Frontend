# FrontDoc-03-Conv：前端工程约定（目录分层 / BFF 路由 / 组件体系 / 状态管理 / 测试 i18n 性能）

> 更新人：3yearsZ
> 更新日：2026-08-20
> 版本：1.0.1（版本基线对齐 1.0.1；Next.js 15 App Router、Zod v3、TanStack Query、shadcn/ui + Tailwind、Vitest + Playwright、i18n next-international 三语）
> Diátaxis：R（Reference · 规范参考 · 前端实现级约定 L2 唯一权威）
...
>
> **SSOT（唯一权威）声明**：本文档是 CS-Web-Frontend **前端 L2 实现级约定**的唯一权威输入。跨仓通用工程约定（命名门禁、版本三源、通用安全红线、Makefile 9 入口）以 [RootDoc-EngConv.md](../../../docs/RootDoc-EngConv.md) 为权威；前端安全红线（BFF Origin 校验、Cookie 托管、路由保护、CSP、前端不持有密钥）以 [FrontDoc-02-Sec.md](FrontDoc-02-Sec.md) 为权威；前端架构 Arc42 完整章节（上下文、构建块、部署、ADR）以 [FrontDoc-01-Arch.md](FrontDoc-01-Arch.md) 为权威；前端 UI 组件库与设计系统以 [RootDoc-UIStandard.md](../../../docs/RootDoc-UIStandard.md) 为权威。
>
> **关联索引**：前端架构总览 → [FrontDoc-01-Arch.md](FrontDoc-01-Arch.md)；前端安全红线 → [FrontDoc-02-Sec.md](FrontDoc-02-Sec.md)；UI 标准与 shadcn 组件 → [RootDoc-UIStandard.md](../../../docs/RootDoc-UIStandard.md)；跨仓通用约定 → [RootDoc-EngConv.md](../../../docs/RootDoc-EngConv.md)；命名门禁 → [RootDoc-ModuleMap.md](../../../docs/RootDoc-ModuleMap.md)；后端安全实现 → [BackDoc-02-Sec.md](../../../CS-Web-Backend/tools/docs/BackDoc-02-Sec.md)
...
|------|------|-----------|--------------|------------|----------|-------------|
| §1 | 目录结构分层（App Router + BFF + 契约层 + 组件层） | 10 | 8 | 5 | 3 | `app/`、`server/`、`src/lib/api/`、`src/components/`、`src/hooks/`、`src/store/` |
| §2 | BFF 路由规范（转发 + Zod 校验 + 权限 preload + 错误翻译） | 9 | 7 | 5 | 3 | `server/bff.ts`、`app/api/[domain]/route.ts`、`server/zodSchemas.ts`、`middleware.ts` |
| §3 | 组件约定（shadcn 扩展 / Props 命名 / 样式系统 / i18n 接入） | 10 | 8 | 5 | 2 | `src/components/ui/`、`src/components/[domain]/`、`tailwind.config.ts`、`postcss.config.js` |
| §4 | 状态管理：TanStack Query + Zustand + Query keys 规范 | 7 | 6 | 4 | 2 | `src/lib/queryClient.ts`、`src/hooks/use{Domain}*.ts`、`src/store/`、`@tanstack/react-query` |
| §5 | 测试分层（Vitest/Playwright）+ i18n + 性能实践 | 8 | 7 | 5 | 2 | `tests/unit/`、`tests/e2e/`、`src/i18n/`、`next.config.mjs`、`server/monitoring.ts` |
| §6 | — | **44（合计）** | **36（合计）** | **24（合计）** | **12（合计）** | — |
...
## 5. 测试分层（Vitest / Playwright）+ i18n + 性能实践
...
> 每次提交涉及目录结构、BFF 路由、组件体系、状态管理、测试、i18n、性能的任何变更前，提交人 MUST 逐项自查并在 PR 描述打钩；CR 审核人 MUST 核对并在未打钩时打回。
...
> ↩ **返回前端架构总览**：[FrontDoc-01-Arch.md](FrontDoc-01-Arch.md) · **前端安全红线**：[FrontDoc-02-Sec.md](FrontDoc-02-Sec.md) · **UI 标准**：[RootDoc-UIStandard.md](../../../docs/RootDoc-UIStandard.md) · **跨仓通用约定**：[RootDoc-EngConv.md](../../../docs/RootDoc-EngConv.md) · **命名门禁**：[RootDoc-ModuleMap.md](../../../docs/RootDoc-ModuleMap.md)
